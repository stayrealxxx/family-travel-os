(()=>{
  ...
  section.innerHTML=`<div class="weather-shell"><div class="weather-head"><div><h3>Weather deck</h3><p id="weatherModeText">实时当地天气 · 临近旅行后自动切换到行程日期预报</p></div><span class="weather-live" id="weatherSource">LIVE</span></div><div class="weather-grid" id="weatherGrid"><div class="weather-skeleton"></div><div class="weather-skeleton"></div></div><div class="weather-alert" id="weatherAlert" hidden></div><div class="weather-footer"><span id="weatherUpdated">正在更新天气…</span><span>Weather data by <a href="https://open-meteo.com/" target="_blank" rel="noreferrer">Open-Meteo</a></span></div></div>`;
  ...
  const fmtDate=s=>{const d=new Date(s+'T12:00:00');return new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric'}).format(d)};
  const dayName=s=>{const d=new Date(s+'T12:00:00');return new Intl.DateTimeFormat('en-US',{weekday:'short'}).format(d)};
  ...
  function localTime(data){try{return new Intl.DateTimeFormat('en-US',{hour:'numeric',minute:'2-digit',timeZone:data.timezone}).format(new Date())}catch(e){return'Local time'}}
  function dailyRows(key,data){
    ...
    return idx.slice(0,4).map(i=>({date:d.time[i],code:d.weather_code[i],hi:Math.round(d.temperature_2m_max[i]),lo:Math.round(d.temperature_2m_min[i]),rain:Math.round(d.precipitation_probability_max[i]||0),wind:Math.round(d.wind_speed_10m_max[i]||0)}));
  }
  function card(key){
    ...
    return `<article class="weather-card ${key===firstStop()?'route-next':''}"><div class="weather-city">${loc.name}<span>${loc.sub}</span></div><div class="weather-now"><div class="weather-icon">${icon}</div><div><div class="weather-temp">${Math.round(cur.temperature_2m)}°</div><div class="weather-condition">${cond} · 体感 ${Math.round(cur.apparent_temperature)}°F</div></div><div class="weather-local"><b>${localTime(data)}</b><span>${src==='cached'?'Cached':'Local time'}</span></div></div><div class="weather-stats"><div class="weather-stat"><small>RAIN</small><b>${Math.round(todayRain)}%</b></div><div class="weather-stat"><small>WIND</small><b>${Math.round(cur.wind_speed_10m||0)} mph</b></div><div class="weather-stat"><small>TRIP</small><b>${ready?'FORECAST':'WAIT'}</b></div></div><div class="weather-forecast">${rows.map(r=>{const wi=wmo(r.code)[0];return `<div class="weather-day"><div class="d">${ready?fmtDate(r.date):dayName(r.date)}</div><div class="i">${wi}</div><div class="t">${r.hi}° / ${r.lo}°</div><div class="r">💧 ${r.rain}%</div></div>`}).join('')}</div><div class="weather-trip-note">${ready?`<strong>Trip forecast active</strong> · ${fmtDate(start)}–${fmtDate(end)} 的行程天气已进入可预报窗口。`:`目前显示当地实时天气与未来几天。<strong>${fmtDate(start)}–${fmtDate(end)}</strong> 的正式行程天气会在临近约两周时自动替换这里。`}</div></article>`;
  }
  function alerts(){
    const msgs=[];
    ['sj','to'].forEach(k=>{if(!forecastReady(k)||!state.data[k])return;dailyRows(k,state.data[k]).forEach(r=>{if(r.rain>=55)msgs.push(`${LOCATIONS[k].name} ${fmtDate(r.date)} 降水概率 ${r.rain}%`);if(r.wind>=25)msgs.push(`${LOCATIONS[k].name} ${fmtDate(r.date)} 最大风速约 ${r.wind} mph`);if(r.lo<=40)msgs.push(`${LOCATIONS[k].name} ${fmtDate(r.date)} 低温约 ${r.lo}°F`)})});
  ...
  function render(){
    ...
    const live=Object.values(state.source).includes('live');document.getElementById('weatherUpdated').textContent=live?'刚刚更新 · °F / mph':'使用上次缓存的天气数据';
  }
